...
namespace FfmpegClient
{
    class Program
    {
        ...
        private static void ExecuteWorkload(List<string> filePaths)
        {
            ...
            var filesToUpload = RemoveFilesAlreadyInCloud(filePaths);

            Parallel.ForEach(filesToUpload, Upload);
        ...
        private static List<string> RemoveFilesAlreadyInCloud(List<string> filePaths)
        {
            var filesNotInCloud = new List<string>(filePaths);

            Console.WriteLine("Checking for existing files");

            var filesAlreadyInCloud = _client.GetExistingFiles(filePaths).Result;
            foreach (var file in filesAlreadyInCloud)
            {
                Console.WriteLine(file + " is already up to date - skipping");
                filesNotInCloud.Remove(file);
            }
            
            Console.WriteLine("Finished checking for existing files");

            return filesNotInCloud;
        }
