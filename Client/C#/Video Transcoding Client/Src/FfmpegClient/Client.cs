...
namespace FfmpegClient
{
    public class Client
    {
        ...
        public async Task<List<string>> GetExistingFiles(List<string> filePaths)
        {
            var existingFileQuery = new FilesByNameQuery { Names = filePaths.Select(Path.GetFileName).ToList() };
            var response = await _httpClient.PostAsJsonAsync("/api/files/query/byname", existingFileQuery);

            EnsureSucceeded(response);

            var result = new List<string>();

            var existingFiles = await response.Content.ReadAsAsync<UserFileList>();

            foreach (var existingFile in existingFiles.Files)
            {
                var localFile = filePaths.First(p => existingFile.Name.Equals(Path.GetFileName(p), StringComparison.OrdinalIgnoreCase));

                var cloudTimestamp = existingFile.LastModifiedTime;
                var localTimestamp = File.GetLastWriteTimeUtc(localFile);

                if (Math.Abs((cloudTimestamp - localTimestamp).TotalMilliseconds) < 1)  // allow for imprecise time storage in cloud
                {
